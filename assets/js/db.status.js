...
// Javascript 숫자에 천단위로 콤마(,) 찍기 thousands() 함수 추가 From prototype.js
	...
	line.success = function(res){
		...
		$scope.seriesMonthly = ['Visitors', 'MEAN'];
		...
		$scope.seriesDaily = ['Visitors', 'MEAN'];
	...
	line.customSuccess = function(res){
		...
		$scope.seriesCustom = ['Visitors', 'MEAN'];

		$scope.labelsCustom = levels
		$scope.custom = {
			total: res.data.TOTAL.thousands(),
			meanbymonth: parseInt( res.data.TOTAL / datas.length ).thousands(),
			meanbyday: parseInt( res.data.TOTAL / not_zero_days ).thousands(),
	...
	function urlLine(begin, end){
		if(begin && end){

			return api.rooturl + "visitor-count" + "?start-date=" + begin + "&end-date=" + end
	...
	function customDefault(){
		$scope.customBegin = get_date("start", $scope.enddate)
		$scope.customEnd = get_date("end", $scope.enddate)
	}
	...
	$scope.customRebuild = function(){
		$http.get(urlLine( $scope.customBegin, $scope.customEnd ), api.conf).then( line.customSuccess, fail )
	}
